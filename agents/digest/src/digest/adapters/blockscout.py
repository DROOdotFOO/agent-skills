...
class BlockscoutAdapter:
    ...
    def fetch(self, query: ExpandedQuery, days: int, limit: int = 50) -> list[Item]:
        ...
        search API to find addresses/tokens matching query terms.
        """
        terms = query.terms
        items: list[Item] = []

        for term in terms[:3]:
            items.extend(self._search_and_fetch(term, limit // max(len(terms[:3]), 1)))
    ...
    def _search_and_fetch(self, term: str, limit: int) -> list[Item]:
        ...
        for result in results:
            ...
            if result_type == "token":
                items.extend(
                    self._fetch_token_transfers(base_url, result, limit // max(len(results), 1))
                )
            elif result_type == "address":
                items.extend(
                    self._fetch_address_txs(base_url, result, limit // max(len(results), 1))
                )
